...
send_report() {
    local url=$1
    local user_agent=${default_headers[$RANDOM % ${#default_headers[@]}]}
    local accept_language=${accept_languages[$RANDOM % ${#accept_languages[@]}]}
    ...
    local response
    response=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$url" -H "${headers[@]}")
    ...
    if [ "$response" -eq 200 ]; then
        ...
        echo "[+] Reports sent: $total_reports (${reports_per_second} reports/min) Current User-Agent: $user_agent Current Accept-Language: $accept_language"
    else
        echo "[-] Report failed"
    fi
}

# Main loop to send reports
while true; do
    send_report "$1"
    sleep 1
done
